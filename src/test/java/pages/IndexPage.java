...
public class IndexPage {
    ...
    public FinancialServicesPage clickFinancialServicesLink() {
        if (financialServicesLink.isDisplayed() && financialServicesLink.isEnabled()) {
            log.info("Financial Services Link is displayed and enabled");
        }
    ...
    public long getIndustriesLinksCount() {
        log.info("Industries Links Count: " + getIndustriesTitles().size());
        return getIndustriesTitlesList().size();

    }

    public boolean verifyFinancialServicesItemDisplayed(String financialServicesItem) {
        log.info("Financial Services Item: " + financialServicesItem);
        return getFinancialServicesItemList().stream().anyMatch(s -> s.getText().equalsIgnoreCase(financialServicesItem));
    }
}
